...
#        optimizer = SGD(lr=0.01, momentum=0.9, nesterov=True)
    ...
    def two_stream_fuse(self):
        ...
        outputs = concatenate([cnn_spatial_multi.output, cnn_temporal_multi.output])
        ...
        model = Model([cnn_spatial_multi.input, cnn_temporal_multi.input], base_mode(x))
    ...
    def cnn_spatial_multi(self):
        ...
        cnn_spatial = Model(inputs=cnn_spatial.input, outputs=cnn_spatial.get_layer('dense_1').output)
    ...
    def cnn_temporal_multi(self):
        ...
        for layer in cnn_temporal.layers:
            layer.trainable = False
        cnn_temporal = Model(inputs=cnn_temporal.input, outputs=cnn_temporal.get_layer('dense_4').output)
        # building inputs and output
        model = Sequential()
