...
class DataSet():
    def __init__(self, class_limit=None, image_shape=(224, 224), original_image_shape=(341, 256), n_snip=5, opt_flow_len=10, batch_size=16):
        ...
        self.static_frame_path = os.path.join(ROOT_DIR + '/data/ucf/test')
        self.train_static_frame_path = os.path.join(ROOT_DIR + '/data/ucf/train')
        self.opt_flow_path = os.path.join(ROOT_DIR + '/data/ucf', 'opt_flow')
        ...
        self.n_train_batch = len(self.train_data_list) // self.batch_size

    @staticmethod
    def get_data_list():
        """Load our data list from file."""
        with open(os.path.join( ROOT_DIR + '/data/ucf', 'data_list.csv'), 'r') as fin:
    ...
    def validation_generator(self):
        ...
        while 1:
            ...
            for row in batch_list:
                ...
                X_spatial, X_temporal = self.get_static_frame_and_stacked_opt_flows(row, False)
    ...
    def train_generator(self):
        ...
        print("\nCreating train generator with %d samples.\n" % len(self.train_data_list))

        idx = 0
        while 1:
            idx += 1
            idx = idx % self.n_train_batch
            # print("\nGenerating batch number {0}/{1} ...".format(idx, self.n_train_batch))
            ...
            # Get a list of batch-size samples.
            batch_list = self.train_data_list[idx * self.batch_size: (idx + 1) * self.batch_size]

            for row in batch_list:
                # Get the stacked optical flows from disk.
                X_spatial, X_temporal = self.get_static_frame_and_stacked_opt_flows(row, True)
    ...
    def get_static_frame_and_stacked_opt_flows(self, row, is_train):
        static_frames = []
        opt_flow_stacks = []

        if is_train:
            static_frame_dir = os.path.join(self.train_static_frame_path, row[1], row[2])
        else:
